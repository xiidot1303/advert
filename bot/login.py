...
def select_lang(update, context):
    ...
    if "UZ" in text:
        BotUser.objects.get_or_create(user_id=update.message.chat.id)
        obj = BotUser.objects.get(user_id=update.message.chat.id)
        obj.lang = "uz"
        obj.save()
        update.message.reply_text(
            get_word("type name", update),
            reply_markup=ReplyKeyboardMarkup(
                [[get_word("back", update)]], resize_keyboard=True
            ),
        )
        return SEND_NAME
    elif "RU" in text:
...
def send_name(update, context):
    ...
    update.message.reply_text(
        ...
        reply_markup=ReplyKeyboardMarkup(
            [[i_contact], [get_word("back", update)]], resize_keyboard=True
        ),
    )
    return SEND_CONTACT
