...
if ENVIRONMENT != "local":  # in production
    ...
    updater = Updater(
        token=TELEGRAM_BOT_API_TOKEN, use_context=True, persistence=persistence
    )
...
settings_handler = ConversationHandler(
    ...
    states={
        ...
        LANG_SETTINGS: [
            CallbackQueryHandler(lang_settings),
            CommandHandler("start", lang_settings),
        ],
...
buyer_handler = ConversationHandler(
    ...
    states={TYPE_POST_NUMBER: [MessageHandler(Filters.text, send_post)]},
    fallbacks=[],
    name="buyer",
    persistent=True,
)
...
vacancy_handler = ConversationHandler(
    ...
    states={
        VACANCY_ACTIONS: [
            CallbackQueryHandler(vacancy_actions),
            MessageHandler(Filters.text, vacancy_actions),
        ],
        # CHANING
        CHANGING: [
            MessageHandler(Filters.text, loop_changing),
            MessageHandler(Filters.photo, loop_changing),
        ],
        CHANGE_PAYMENT: [
            MessageHandler(Filters.text, change_payment),
            MessageHandler(Filters.photo, change_payment),
        ],
