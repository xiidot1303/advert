    ...
    if update.callback_query:
        ...
        if 'publishstatement' in data:
            *args, id = str(data).split('_')
            ...
            st_obj.save()
            bot.answer_callback_query(callback_query_id=update.id, text=get_word('completed answering', update), show_alert=True)
            return
            ...
            *args, id = str(data).split('_')
            ...
            if not answer.answer:
                update.message.reply_text(get_word('send text vacancy', update), 
            ...
            text = '{}\n\n{}: *{}*'.format(question_obj.question, 
                get_word('current answer', update), split_by_slash(answer.answer)[0])
            if question_obj.variants:
                keyboards = [[get_word('stay current answer', update)]] + get_variants_for_buttons(question_obj.variants)
            else:
...
def loop_changing(update, context):
    ...
    question_index = len(split_by_slash(answer_obj.new_answer)) + 1   # get next question by counting current answers
    ...
    if update.message.text == get_word('back', update):
        ...
        question_obj = get_backup_question(question_index, answer_obj.pk)
        text = '{}\n\n{}: *{}*'.format(question_obj.question, 
            get_word('current answer', update), split_by_slash(answer_obj.answer)[question_index-1])
        if question_obj.variants:
            keyboards = [[get_word('stay current answer', update)]] + get_variants_for_buttons(question_obj.variants)
        else:
            keyboards = [[get_word('stay current answer', update)]]
    ...
    if last_question.index == question_index:
        ...
        update.message.reply_text(get_word('completed answering', update).replace('[id]', str(st_obj.pk)))
        vacancies(update, context)
        return VACANCY_ACTIONS


    next_question_obj = get_backup_question(question_index+1, answer_obj.pk)
    text = '{}\n\n{}: *{}*'.format(next_question_obj.question, 
        get_word('current answer', update), split_by_slash(answer_obj.answer)[question_index])
    if next_question_obj.variants:
        keyboards = [[get_word('stay current answer', update)]] + get_variants_for_buttons(next_question_obj.variants)
    else:
        keyboards = [[get_word('stay current answer', update)]]
