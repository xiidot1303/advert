...
def card(request, pk):
    ...
    if not answer_obj.answer:
        ...
        context = {'st_pk': st_pk, 'name': name, 'phone_number': phone_number, 'date': date, 'text': text}
    ...
    context = {'questions': questions, 'answers': answer_list, 'photo': photo, 'st_pk': st_pk, 'name': name, 
        'phone_number': phone_number, 'date': date, 'answer_pk': answer_obj.pk}
...
class AnswerEditView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['questions'] = Backup_question.objects.filter(answer = context['object'].pk)
        return context
            ...
            for q in Question.objects.all().order_by('index'):
                if user.lang == 'uz':
                    Backup_question.objects.create(user = user, question = q.questionuz, variants = q.variantsuz, 
                        index = n, req_photo = q.req_photo, is_required = q.is_required, answer = answer_obj.pk)
                else:
